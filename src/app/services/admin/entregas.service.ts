...
export class EntregasService {
  ...
  public buscar(id: number): Observable<IResultDataEntrega> {
    return this._restService.get<IResultDataEntrega>(
      `${this.apiUrl}/buscar/${id}`,
    );
  }

  public confirmar(id: number): Observable<IResultDataCreate> {
    return this._restService.put<IResultDataCreate>(
      `${this.apiUrl}/confirmar/${id}`,
      {},
    );
  }
  ...
  public listarPedidosDisponibles(): Observable<{
    data: IPedidoEntregaDisponible[];
  }> {
    return this._restService.get<{ data: IPedidoEntregaDisponible[] }>(
      `${this.apiUrl}/pedidos/disponibles`,
    );
  }

  public obtenerPedidoPendiente(
    id: number,
  ): Observable<{ data: IPedidoEntregaPendiente[] }> {
    return this._restService.get<{ data: IPedidoEntregaPendiente[] }>(
      `${this.apiUrl}/pedidos/${id}/pendientes`,
    );
  }

  public listarProveedoresEmpresa(
    idEmpresa: number,
  ): Observable<IComboBoxOption[]> {
    return this._restService.get<IComboBoxOption[]>(
      `${this.apiUrl}/proveedores/empresa/${idEmpresa}`,
    );
  }
