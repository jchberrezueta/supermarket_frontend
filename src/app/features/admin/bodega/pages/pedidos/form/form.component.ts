...
export default class FormComponent implements OnInit {
  ...
  protected confirmarCabecera(): void {
    if (
      this.formData.controls.ideEmpr.invalid ||
      this.formData.controls.motivoPedi.invalid ||
      this.formData.controls.fechaEntrPedi.invalid
    ) {
      this.alerta(
        'Cabecera incompleta',
        'Seleccione la empresa proveedora, el motivo y la fecha esperada de entrega.',
      );
      return;
    }
    if (
      this.formData.controls.fechaEntrPedi.value! < this.fechaPedidoCalendario
    ) {
      this.alerta(
        'Fecha esperada inválida',
  ...
  protected agregarProducto(): void {
    ...
    if (
      !Number.isInteger(this.cantidadProducto) ||
    ...
    if (
      !precio ||
      Number(precio.ide_empr) !== Number(this.formData.controls.ideEmpr.value)
    ) {
      this.alerta(
        'Producto inválido',
        'Seleccione un producto ofrecido por la empresa.',
      );
      return;
    }
    if (this.detalles.some((detalle) => detalle.ideProd === precio.ide_prod)) {
      this.alerta(
        'Producto duplicado',
        'Cada producto solo puede aparecer una vez en el pedido.',
      );
      return;
    }
    if (
      this.preview.dctoCompraProd + this.preview.dctoCaducProd >
      this.preview.subtotalProd
    ) {
      this.alerta(
        'Descuentos inválidos',
        'Los descuentos no pueden superar el subtotal bruto.',
      );
      return;
    }
    this.detalles = [
  ...
  protected guardarCambiosProducto(): void {
    ...
    if (!clave) return;
    const indice = this.detalles.findIndex(
      (detalle) => detalle.claveLocal === clave,
    );
    if (indice < 0) {
    ...
    if (
      this.detalles.some(
        (detalle) =>
          detalle.claveLocal !== clave && detalle.ideProd === precio.ide_prod,
      )
    ) {
      this.alerta(
        'Producto duplicado',
    ...
    this.detalles = this.detalles.map((detalle, itemIndex) =>
      itemIndex === indice ? actualizado : detalle,
    );
    this.cancelarEdicionProducto();
  }
  ...
  protected eliminarProducto(claveLocal: string): void {
    this.detalles = this.detalles.filter(
      (detalle) => detalle.claveLocal !== claveLocal,
    );
  ...
  protected guardar(): void {
    if (this.claveEdicion) {
      this.alerta(
        'Edición de producto pendiente',
        'Guarde o cancele la edición del producto antes de continuar.',
      );
      return;
    }
    ...
    request
      ...
      .subscribe({
        next: (res) => {
          if (Number(res?.p_result) !== 1) {
            this.alerta(
              ...
              this.mensajeRespuesta(
                res?.p_response,
                'Revise la información del pedido.',
              ),
              'error',
            );
            return;
          ...
          Swal.fire({
            icon: 'success',
            title: this.isAdd ? 'Borrador guardado' : 'Borrador actualizado',
          }).then(() => this.router.navigate(['/admin/bodega/pedidos/list']));
        },
        error: (error) =>
          this.alerta(
            'No se pudo guardar',
            error?.error?.message ?? 'Ocurrió un error al guardar el borrador.',
            'error',
          ),
      });
  }
  ...
  protected get cantidadTotal(): number {
    return this.detalles.reduce((sum, d) => sum + d.cantidadProd, 0);
  }
  ...
  protected get subtotalTotal(): number {
    return this.redondear(
      this.detalles.reduce((sum, d) => sum + d.subtotalProd, 0),
    );
  }
  protected get descuentosTotal(): number {
    return this.redondear(
      this.detalles.reduce(
        (sum, d) => sum + d.dctoCompraProd + d.dctoCaducProd,
        0,
      ),
    );
  }
  protected get ivaTotal(): number {
    return this.redondear(this.detalles.reduce((sum, d) => sum + d.ivaProd, 0));
  }
  protected get totalGeneral(): number {
    return this.redondear(
      this.detalles.reduce((sum, d) => sum + d.totalProd, 0),
    );
  }
  ...
  private cargarBorrador(id: number): void {
    ...
    this.pedidosService.buscar(id).subscribe({
      next: (res) => {
        ...
        this.fechaPedidoCalendario = this.extraerFechaCalendario(
          pedido.fecha_pedi,
        );
        ...
        this.cargarProductosEmpresa(pedido.ide_empr, () =>
  ...
  private cargarProductosEmpresa(
    ...
    this.empresasService
      .listarPreciosProductosEmpresa(ideEmpr)
      ...
      .subscribe({
        next: (res) => {
          ...
          this.productos = this.preciosEmpresa.map((precio) => ({
            value: precio.ide_prod,
            label: precio.nombre_prod!,
          }));
          completado?.();
        },
        error: () => {
  ...
  private cargarDetalles(id: number): void {
    this.pedidosService
      ...
      .subscribe({
        next: (res) => {
          this.detalles = (
            (res.data ?? []) as unknown as IDetallePedidoResult[]
          ).map((detalle) => ({
            claveLocal: `persistida-${detalle.ide_deta_pedi}`,
            ideProd: detalle.ide_prod,
            nombreProd:
              detalle.nombre_prod ??
              this.preciosEmpresa.find((p) => p.ide_prod === detalle.ide_prod)
                ?.nombre_prod ??
              `Producto #${detalle.ide_prod}`,
            cantidadProd: detalle.cantidad_prod,
            precioUnitarioProd: detalle.precio_unitario_prod,
            subtotalProd: this.redondear(
              detalle.precio_unitario_prod * detalle.cantidad_prod,
            ),
  ...
  private get precioSeleccionado(): IEmpresaPreciosResult | undefined {
    return this.preciosEmpresa.find(
      (precio) => precio.ide_prod === this.productoSeleccionado,
    );
  }
  ...
  private extraerFechaCalendario(value: string): string {
    ...
    return local
      ...
      : this.fechaCalendarioLocal(new Date());
  }
  private alerta(
    title: string,
    text: string,
    ...
    void Swal.fire({ icon, title, text });
  }
